...
impl WindowsFirewallManager {
    ...
    unsafe fn add_app_rule(
        ...
        rule.SetDescription(&BSTR::from(format!(
            "Allow VNT {} traffic for {}",
            protocol_name, self.device_name
        )))?;
    ...
    unsafe fn add_interface_rule(
        &self,
        rules: &INetFwRules,
        direction: &str,
        dir_value: NET_FW_RULE_DIRECTION,
    ) -> windows::core::Result<()> {
        ...
        rule.SetName(&BSTR::from(&rule_name))?;
        rule.SetDescription(&BSTR::from(format!(
            "Allow all traffic on VNT interface {}",
            self.device_name
        )))?;
