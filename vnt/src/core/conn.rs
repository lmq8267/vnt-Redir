...
impl VntInner {
    ...
    pub(crate) fn new_device0<Call: VntCallback, Device: DeviceWrite>(
        config: Config,
        callback: Call,
        device: Device,
    ) -> anyhow::Result<Self> {
...
impl VntInner {
    ...
    pub fn stop(&self) {
        ...
        {
            let device_name = self.config.device_name.as_deref().unwrap_or("vnt-tun");

            // 清理防火墙规则
            let firewall_manager =
                crate::tun_tap_device::windows_firewall::WindowsFirewallManager::new(device_name);
            ...
            if let Err(e) = adapter_manager.remove_adapter() {
                log::warn!("删除虚拟网卡失败: {:?}", e);
            }
        }
...
impl Drop for VntInner {
    fn drop(&mut self) {
        ...
        #[cfg(all(target_os = "windows", feature = "integrated_tun"))]
        {
            ...
            let adapter_manager =
                crate::tun_tap_device::windows_adapter::WindowsAdapterManager::new(device_name);
            let _ = adapter_manager.remove_adapter();
        }
